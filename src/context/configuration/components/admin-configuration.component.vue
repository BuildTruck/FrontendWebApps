...
<template>
  <div class="config-form">
    ...
    <!-- Sección de Notificaciones de Administrador -->
    <div class="config-section">
      <h3 class="config-title">{{ $t('settings.adminNotificationsSection') }}</h3>
      <p class="config-description">{{ $t('settings.adminNotificationsDescription') }}</p>

      <AppInput
          v-model="settings.admin_notifications"
          type="select"
          :label="$t('settings.adminNotifications')"
          :options="[
            { label: $t('general.yes'), value: 'true' },
            { label: $t('general.no'), value: 'false' }
          ]"
      ...
      <AppInput
          v-model="settings.system_alerts"
          type="select"
